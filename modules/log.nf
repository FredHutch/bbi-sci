...
process finish_log {
   publishDir path: "${params.output_dir}/", saveAs: save_logs, pattern: "*.log", mode: 'copy', overwrite: true
   publishDir path: "${params.output_dir}/", saveAs: save_txt_for_wrap, pattern: "*.txt", mode: 'copy', overwrite: true

   input:
      tuple val(key), file(logfile)
      file exp_dash
...
process zip_up_log_data {
   publishDir path: "${params.output_dir}/exp_dash/js/", pattern: "*.js", mode: 'copy', overwrite: true
